...
float calculateRadiationByTriangle(const Vertex& a, const Vertex& b, const Vertex& c, const Light& light) {
	...
	for (unsigned int i = 0; i < SAMPLE_NUM; i++) {
		float theta = acosf(1.0f - (float)rand() / RAND_MAX);		// acosf(1 - 2 * (float)rand() / RAND_MAX);	由于取半球，所以theta的范围为[0, pi/2]
	...
	// 假设三角面元足够小，相对球面光源近似为一点，计算三角面元的面积
...
int main(int argc, char** argv) {
	...
	Light light{
		...
		0.0f,
		1.0f
	};			// 光源位置，光源方向，光源半径，光照强度。这里只考虑位置和光照强度
